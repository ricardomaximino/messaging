...
public class OperatorServiceImpl implements OperatorService{
	...
	public Flux<Operator> findBySkills(String skillsId) {
		return operatorRepository.findAll().filter(o -> {
			if(o.getSkillsList()!= null && !o.getSkillsList().isEmpty()) {
				for(Skills s : o.getSkillsList()) {
					if(s.getId().equals(skillsId)) return true;
				}
				return false;
			}else {
				return false;
